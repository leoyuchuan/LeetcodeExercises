package com.leoyuchuan.leetcode.wordladder;
...
public class WordLadder {
	public static void main(String[] args) {
		...
		// wordList.addAll(Arrays.asList(new String[] { "si", "go", "se", "cm",
		// "so", "ph", "mt", "db", "mb", "sb", "kr",
		// "ln", "tm", "le", "av", "sm", "ar", "ci", "ca", "br", "ti", "ba",
		// "to", "ra", "fa", "yo", "ow", "sn",
		// "ya", "cr", "po", "fe", "ho", "ma", "re", "or", "rn", "au", "ur",
		// "rh", "sr", "tc", "lt", "lo", "as",
		// "fr", "nb", "yb", "if", "pb", "ge", "th", "pm", "rb", "sh", "co",
		// "ga", "li", "ha", "hz", "no", "bi",
		// "di", "hi", "qa", "pi", "os", "uh", "wm", "an", "me", "mo", "na",
		// "la", "st", "er", "sc", "ne", "mn",
		// "mi", "am", "ex", "pt", "io", "be", "fm", "ta", "tb", "ni", "mr",
		// "pa", "he", "lr", "sq", "ye" }));
		// System.out.println(ladderLength("qa", "sq", wordList));

		wordList.addAll(Arrays.asList(new String[] { "slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks",
				"gang", "pool", "mock", "fort", "heel", "send", "ship", "cols", "alec", "foal", "nabs", "gaze", "giza",
				"mays", "dogs", "karo", "cums", "jedi", "webb", "lend", "mire", "jose", "catt", "grow", "toss", "magi",
				...
				"torn", "cups", "worm", "baum", "axon", "purr", "vise", "grew", "govs", "meat", "chef", "rest",
				"lame" }));

		long a = System.nanoTime();
		int length = ladderLength("sand", "acne", wordList);
		long b = System.nanoTime();
		System.out.println((b - a) / (1000000.0) + "ms");

		// wordList.addAll(Arrays.asList(new String[] { "ac", "cc", "ab" }));
		// System.out.println(ladderLength("ab", "cd", wordList));
	}

	public static int ladderLength(String beginWord, String endWord, Set<String> wordList) {
		if (beginWord.equals(endWord))
			return 0;
		if (wordDifference(beginWord, endWord) == 1) {
			return 2;
		}
		...
		for (String s : wordList) {
			if (wordDifference(beginWord, s) == 0) {
				clone.remove(s);
			} else if (wordDifference(beginWord, s) == 1) {
				selectedWords.add(s);
				clone.remove(s);
			}
		}
		...
		for (String s : selectedWords) {
			// HashSet<String> cloneset = ((HashSet<String>) (clone.clone()));
			int length = ladderLength(s, endWord, wordList);
			if (length == 0) {
				continue;
			}
	...
	public static int wordDifference(String a, String b) {
		int difference = 0;
	...
	static class Tree {
		...
		public Tree(String val, int capacity) {
			this.val = val;
			children = new Tree[capacity];
		}
		...
		void resetChildren(int capacity) {
			children = new Tree[capacity];
		}

		@Override
		public boolean equals(Object obj) {
			if (obj.getClass().equals(Tree.class.getClass())) {
				throw new ClassCastException(obj.toString() + " is not an instance of class " + Tree.class.toString());
			}
			return val.equals(((Tree) obj).val);
		}
	}
}
